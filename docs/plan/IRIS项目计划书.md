...
> 版本：v1.2，2026-07-08（v1.1 2026-07-07；v1.0 2026-07-02）。取代 01/02 号文档中的 "RevAct" 定位（贡献结构不变，世界模型叙事升为显式主线）。**v1.1 变更（代码已落地）**：P0 训练/部署 prompt 格式统一（§5.1 规定的 H_t 历史此前漏在代码外，已落地并与 rollout 策略共用一个 builder）；P1 多轮轨迹样本；grounded-reward RL（GRPO/GSPO）从"附录待做"变为"已实现的 ablation"（§7 Stage 3）；守护式 rollout FSR 评测基础设施上线并产出 pilot 数字（§9.6）。RL 的定位仍是 ablation、非主线贡献。
> **v1.2 变更（P2，代码已落地，样本格式升级为 `iris.v2`）**：① **输入侧**——H_t 历史行升级为"动作 + 观测 delta + 变化标记"三要素（§5.1.1），标记只来自部署时可观测的量（杜绝把 grounded 标签泄入历史造成训练-部署错位），`[no-effect]` 显式标注"动作没改变任何东西"，修复"动作无效 → 后续每步输入完全相同 → 策略死循环"的失败模式；② **输出侧**——逆向世界模型补上 reasoning：新增 `<rev_check>`（label 之前的机制级 undo 通道检查，事实由探针 undo 实录钉死）并落地 §5.1 原已承诺但代码缺失的 `<undo>` 字段（含实测步数），离散标签不再是无理由的分类目标（rationale 蒸馏优于纯 label 蒸馏，Hsieh et al. 2023）；③ **prompt 治理**——全部 LLM prompt（采集策略/teacher 蒸馏/agent 系统提示词）与目标模板池入 registry，工作台可编辑，样本 meta 记 `prompts_fp` 指纹溯源（§5.5）。
...
IRIS 做三件事：（i）**操作化定义并行为测量可逆性**——对每个 (状态, 动作) 在真实环境执行动作、运行站点 undo 控制器、比对后端信号，产出 {REVERSIBLE, REVERSIBLE_WITH_COST(k), PARTIALLY_RECOVERABLE, IRREVERSIBLE, UNKNOWN} 谱系标签与 undo 代价，替代意见标签；（ii）**把 invertibility-aware 世界模型内在化进策略**——单次自回归输出 `<observation>/<reasoning>/<prediction>/<rev_check>/<reversibility>/<undo>/<decision risk=s>/<answer>`，前向预测由真实 O_{t+1} 监督、可逆性/undo 由行为标签监督、`<rev_check>`（逆向机制推理，标签之前）措辞由 teacher 写而事实由 undo 实录钉死、决策由 grounded oracle 监督；（iii）**首个面向安全的 coupling 受控研究**——同基座、同 grounded 标签、同信息下对照 coupled（内在）与 decoupled（WAC/SafePred 式外置 verifier），在安全（FSR/IER/FBR）、成本（calls/tokens）、校准（ECE/risk-coverage）、泛化（cross-action-class/cross-privilege）四个维度回答"内在化买到了什么"。全部实验在 WebArena shopping + shopping_admin 单站点内、8×A800 + Qwen2.5-3B/7B 规模上可完成。
...
单一自回归 Transformer（Qwen2.5-7B-Instruct 正式 / 3B pilot，LoRA）。**输入**：G（含约束，措辞多样化）、H_t（压缩历史，见 §5.1.1）、o_t（当前步**完整** pruned a11y ≤4k tokens）。**候选动作不进输入**（否则退化为外置评估器）。**输出（单次生成，`iris.v2`）**：
...
<rev_check>      逆向世界模型·推理：在给出标签之前，机制级检查"本站/本账户是否存在
                 可执行的 undo 通道"（找的是真实 remove/cancel/reset 控件与实测代价，
                 不是按钮字样联想）。措辞可由 teacher 写，事实由探针 undo 实录钉死。
...
<undo>           undo 计划概要 + 实测步数（如 "remove the item from the cart (1 step)"）
                 或 "none available" / "unverified"  （逆向世界模型·构造）
<decision>       {EXECUTE|VERIFY|CONFIRM|AVOID} risk=s∈[0,1]（校准决策头）
</think>
<answer>         可执行 BrowserGym 动作（decision gating 落地，见 §9）
```

**为什么 `<rev_check>` 必须存在且必须在标签之前**：只有离散 `<reversibility>` 标签时，逆向监督对模型是一个无理由的分类目标——模型可以靠"动作类 → 标签"的查表记忆拟合训练分布，遇到新动作类（cross_action_class split 的主战场）即失效；这正是 rationale-augmented 蒸馏一贯优于 label-only 蒸馏的原因（Distilling Step-by-Step, Hsieh et al. 2023）。`<rev_check>` 把标签锚到**页面上可见的机制证据**（购物车行有 'Remove item' 控件 / 订单页无任何顾客侧 cancel affordance），推理在前、结论在后，自回归生成时标签以推理为条件而非反之。防标签循环不变式照旧：`<rev_check>` 的**事实**（undo 通道是否存在、实测步数、残差）全部来自探针记录的 `undo_actions/undo_steps/residual_diff` 实录，teacher 只许改措辞，QC 矛盾正则拒绝任何与 pinned 标签相悖的说法（§5.2）。

#### 5.1.1 输入格式规范：H_t 历史的三要素行（P2 已落地）

每步输入 = **G + H_t + 当前步完整 o_t**。H_t 每个历史步压缩为一行、三要素：

```text
<history>
1. goto('http://…/product.html') -> [nav] Cell Phone Stand (/product.html)
2. click('1468')                 -> [state-change] cart items 0 -> 1
3. click('1468')                 -> [no-effect] no visible change
```

- **动作**：执行过的动作调用原文；
- **变化标记（flag）**：`nav`（URL 变了）/ `state-change`（后端信号动了：购物车行数、
  订单号集合、地址数、表单值）/ `update`（同 URL 页面内容变化，如菜单展开）/
  `no-effect`（什么都没变）。标记由 `envs.obs_utils.obs_delta` 从**相邻两步真实观测**
  计算（fingerprint 实体信号 diff），**绝不来自 grounded 标签**——部署时没有标签，
  历史里混入标签即训练-部署分布错位；
- **关键 delta**：一行自然语言的变化摘要（优先实体信号如 "cart items 0 -> 1" /
  "new order #193"，退化为页面标题+短 URL）。

`[no-effect]` 是承重设计：没有它，"动作无效 → (G, o_t) 与上一步完全相同 → 策略必然
复读同一动作"是结构性死循环；有了它，第 t 步与第 t+1 步的输入必然不同，且模型被系统
提示词明确要求"看到 [no-effect] 就换动作"。这一版式与主流 agent 训练的输入构造对齐：
AgentOccam（objective + interaction history + 当前 AXTree）、UI-TARS（instruction +
近 N 步 (o,a) 交互史 + 当前截图，thoughts 留存为短期记忆）、BrowserGym GenericAgent
（goal + action history + last error）；"只保留关键状态变化的自然语言摘要"即 WMA 的
transition-focused observation abstraction（其消融证明全量 next-observation 表示反而
有害）。两类样本族共享该规范：**单步样本**（压缩历史 + 完整当前观测）与**多轮样本**
（最近 K=5 步保留为真实 user/assistant 轮、更早步折叠进首轮 `<history>`，与 rollout
的 max_history 压缩镜像；VAGEN/多轮 RL 的标准形态）。合成状态（reach 直达）的历史按
reach 计划诚实渲染并在 meta.history_source 标注 plan/canonical/trajectory 来源。
...
| `<rev_check>` 措辞 | teacher 条件蒸馏；**事实（undo 通道存在性/实测步数/残差）来自探针 undo_actions/undo_steps/residual_diff 实录**，QC 矛盾即拒 | S7(措辞)+环境(事实) ⭐ | ✅ 只写措辞 |
| `<reversibility>` | **execute-then-undo 行为测量**（后端信号复原判定） | 环境 ⭐ | ❌ 绝不 |
| `<undo>` | undo 控制器**实际执行**的动作序列（undo_actions 实录）+ 实测步数 | 环境 ⭐ | ❌ 绝不 |
| `<decision>`+risk | 确定性 oracle：f(grounded 可逆性 × 约束违背 × 是否被请求) | 规则(吃 grounded 输入) | ❌ 绝不 |
| `<answer>` | 专家动作 / 规则安全替代动作 | 轨迹+规则 | ❌ 绝不 |
| H_t 历史行（输入侧） | 相邻真实观测的 fingerprint diff（obs_delta） | 环境 | ❌ 绝不 |
...
离散 `<reversibility>` 保留（可读、可算 macro-F1）；`REVERSIBLE_WITH_COST(k)` 中的 k 与 `<decision>` 的连续 risk 分数支撑校准叙事（risk-coverage、ECE）——离散标签管"是什么"，连续量管"多确定/多贵"；`<rev_check>` 管"凭什么"（机制证据，泛化的载体）。第一版不做 vocab 手术（标签按普通 token 处理，实测 3B 也能稳定跟随格式）。`risk_type/risk_level` **不进监督**（意见性最强、且 WebGuard 已占人工版），只作分析用弱标签。

### 5.5 Prompt 治理与可调优性（P2 已落地）

pipeline 中每一条 LLM-facing prompt 都是实验变量，必须可调、可溯源、不可散落硬编码。
实现（`revact/prompts.py` 的 **prompt registry** + 工作台「Prompt 管理」页）：

| registry 条目 | 用途 | 改动后需要 |
|---|---|---|
| `agent_system` | 训练样本 system 角色 + IrisPolicy 部署（同一条，P0 不变式） | **重跑 assemble/assemble-multiturn/split 重物化** |
| `collector_system` | S2 成功轨迹采集的策略模型（DeepSeek 等） | 之后的新采集 job 即时生效 |
| `teacher_distill` | S7 teacher 条件蒸馏模板（grounded 事实为不可违背占位符） | 之后的新蒸馏 job 即时生效 |
| `explicit/implicit_constraint_templates`、`request_templates` | §6.4 目标措辞池 | 重跑 assemble |

机制：覆盖存 `configs/prompts.local.json`（gitignore，默认值在代码里），**调用时读取**——
子进程 job 自动拿到最新值，无需重启；带占位符校验（缺 `{goal}` 等直接拒绝）。
**溯源**：每条物化样本 meta 记 `prompts_fp`（生效 prompt 集的指纹），dataset card 与
导出目录（`prompts.json` 快照）同步记录——任何一次 prompt 调优产出的数据集都能对上
"是哪套 prompt 生成的"，A/B 两套 prompt 的样本不会混在一起无法归因。安全边界不变：
prompt 只影响**措辞与采集行为**，`<reversibility>/<undo>/<decision>/<answer>` 的结论
仍由环境与规则钉死（§5.2），调 prompt 调不动标签。
...
17. Hsieh et al. Distilling Step-by-Step! Outperforming Larger Language Models with Less Training Data and Smaller Model Sizes. ACL Findings 2023.（§5.1 `<rev_check>` 的依据：rationale+label 蒸馏优于 label-only）<https://arxiv.org/abs/2305.02301>
18. AgentOccam: A Simple Yet Strong Baseline for LLM-Based Web Agents. 2024.（§5.1.1 输入构造对齐：objective + interaction history + 当前 AXTree）<https://arxiv.org/abs/2410.13825>
19. UI-TARS: Pioneering Automated GUI Interaction with Native Agents. 2025.（§5.1.1：近 N 步 (o,a) 交互史作为短期记忆的原生 agent 输入形态）<https://arxiv.org/abs/2501.12326>
